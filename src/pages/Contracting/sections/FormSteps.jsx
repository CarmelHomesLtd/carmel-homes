...
function FormSteps() {
        ...
        return (
          <>
            <textarea
              ...
              placeholder={input.placeholder}
              rows={5}
              className={`bg-white rounded-md p-2 w-full ${formValues["description"] == "" && input.type == "date" ? "text-gray-400" : "text-blue-primary"}`}
            />
          </>
        );
  ...
  function renderStep(step) {
    ...
    return (
      <div key={step.step} className={`w-full p-2 gap-2 ${step.step == currentStep ? "grid" : "opacity-0 pointer-events-none absolute"}`}>
  ...
  return (
    <div className="relative bg-white p-5 pb-10 lg:p-10 w-full lg:w-[clamp(20rem,60%,50rem)] mx-auto rounded-xl shadow-lg lg:shadow-xl shadow-brown-gray/50 flex flex-col justify-between">
      <div className="grid gap-1">
        <h2 className="text-[1.75rem] text-center font-bold">Get Your FREE Estimate Today!</h2>
      </div>
        ...
        !isFormSubmitted
          ? (
            <>
              ...
              {/* Step Form */}
              <form name="free-estimate" method="post" className="z-[2]" onSubmit={handleSubmit(onSubmit, onError)}>
